...
def load_file(uploaded_file):
    # Support both CSV and Excel; for Excel, allow sheet selection
    ...
    if file_type == "csv":
        ...
        sheet = st.selectbox("Select sheet/tab", xls.sheet_names)
...
# -----------------------------
train_file = st.file_uploader("Upload Training (CSV or Excel, multi-tab supported)", type=["xlsx", "csv"])
model_trained = False

if train_file:
    train_df, _ = load_file(train_file)
    ...
    if st.button("Train ESN Model"):
        ...
        if grid_search:
            ...
            best_r2 = -np.inf
            best_params = {}
            # These ranges can be user-modifiable
            reservoir_options = st.multiselect('Reservoir sizes to try', [20, 50, 100, 200, 300], default=[20, 50, 100])
            spectral_options = st.multiselect('Spectral radius values to try', [0.6, 0.8, 1.0, 1.2, 1.5], default=[0.7, 0.9, 1.1])
            prog = st.progress(0)
            trials = list(itertools.product(reservoir_options, spectral_options))
            for i, (n_res_try, sr_try) in enumerate(trials):
...
# -----------------------------
# 4️⃣ Upload Test File (CSV/Excel multi-tab support)
# -----------------------------
test_file  = st.file_uploader("Upload Test (CSV/Excel, multi-tab supported)", type=["xlsx", "csv"], key="test_uploader")

if test_file and 'trained_model' in st.session_state:
    test_df, _ = load_file(test_file)
